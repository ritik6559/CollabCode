...
const CreateRoomForm = ({ open, onOpenChange, roomType }: CreateRoomFormProps) => {
    ...
    const { mutateAsync: createRoom, isPending, error, reset } = useCreateRoom();
            ...
            <DialogContent className="border-stone-100/10 bg-stone-950/95 backdrop-blur-xl sm:max-w-[480px]">
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 pt-2">
                        <FormErrorBanner
                            message={error ? getApiErrorMessage(error, "Failed to create room. Please try again.") : null}
                        />
                                ...
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Textarea
                                            ...
                                            className={`min-h-[90px] resize-none ${inputClasses}`}
                                            {...field}
                                        />
                                    </FormControl>
                                    <FormMessage className="text-sm text-rose-300" />
